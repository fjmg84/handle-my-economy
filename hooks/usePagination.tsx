...
function usePagination(transactions: Transaction[]) {
  let rowPerPage = 5;
  let maxPage = Math.ceil(transactions.length / rowPerPage);
    ...
    return (
      <div id="paginations">
        <div className="flex items-center justify-center">
          ...
          >
            <ArrowLeft color={ disabledPrevious ? "#a1a1aa" : "#4f46e5"} size={20}/>
          </button>
          <button
            className={`p-1 rounded transition ml-2 hover:cursor-pointer hover:disabled:cursor-not-allowed hover:disabled:bg-transparent`}
          ...
          >
            <ArrowRight color={ disabledNext ? "#a1a1aa" : "#4f46e5"}  size={20}/>
          </button>
        </div>
