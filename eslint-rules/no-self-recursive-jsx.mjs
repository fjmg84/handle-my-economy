...
const noSelfRecursiveJsxRule = {
  meta: {
    ...
    messages: {
      noSelfRecursive:
        "El componente '{{name}}' se renderiza a si mismo en JSX, lo que puede causar recursion infinita.",
      noSelfRecursiveCall:
        "El componente '{{name}}' se llama a si mismo, lo que puede causar recursion infinita.",
    },
  },
  create(context) {
